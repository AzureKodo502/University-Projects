...
public class CategoriaBoxController {
    ...
    void AggiungiLeScarpe(ArrayList<Scarpe> scarpe) {
        ...
        for (int i = 0; i < maxSize; i++) {
            ...
            String nomeFile = scarpe.get(i).ImageUrl();

            // Fallback se è null
            if (nomeFile == null || nomeFile.equals("N/A")) {
                // Fallback: prova a usare il vecchio metodo ID + .png
                nomeFile = scarpe.get(i).Id() + ".png";
            }
            ...
            try {
                var resource = HelloApplication.class.getResourceAsStream(url);
                if (resource != null) {
                    immagineProdotti[i].setImage(new Image(resource));
                } else {
                    System.out.println("Immagine non trovata: " + url);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
